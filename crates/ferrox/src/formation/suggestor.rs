...
/// Score weights (integer-scaled for CP-SAT)
const W_CONFIDENCE: i64 = 1000;
...
fn assemble_cp(req: &FormationRequest, catalog: &[ProfileSnapshot]) -> FormationPlan {
    ...
    let eligible: Vec<&ProfileSnapshot> = catalog.iter()
        .filter(|s| req.required_capabilities.iter().all(|cap| s.capabilities.contains(cap)))
        .collect();
    ...
    let mut x: Vec<Vec<i32>> = vec![vec![0i32; n_cands]; n_roles];
    ...
    for i in 0..n_roles {
        let vars: Vec<i32> = (0..n_cands).filter(|&j| x[i][j] != 0).map(|j| x[i][j]).collect();
        if vars.len() > 1 {
    ...
    for j in 0..n_cands {
        let vars: Vec<i32> = (0..n_roles).filter(|&i| x[i][j] != 0).map(|i| x[i][j]).collect();
        if vars.len() > 1 {
            let ones = vec![1i64; vars.len()];
            model.add_linear_le(&vars, &ones, 1);
        }
    ...
    for i in 0..n_roles {
        for j in 0..n_cands {
            if x[i][j] != 0 && solution.value(x[i][j]) == 1 {
    ...
    let unmatched_roles: Vec<_> = req.required_roles.iter()
        .enumerate()
...
fn score(snap: &ProfileSnapshot) -> i64 {
    ...
    let base = (snap.confidence_max * W_CONFIDENCE as f32) as i64;
...
mod tests {
    ...
    fn no_double_booking() {
        ...
        let names: std::collections::HashSet<_> = plan.assignments.iter().map(|a| &a.suggestor).collect();
    ...
    fn capability_filter_respected() {
        ...
        let catalog = vec![
            cap_snap,
            snap("plain", SuggestorRole::Analysis, 0.95),
        ];
        let req = FormationRequest {
            id: "r4".to_string(),
            required_roles: vec![SuggestorRole::Analysis],
