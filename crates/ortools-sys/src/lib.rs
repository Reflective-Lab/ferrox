...
#[repr(C)] pub struct CpModelBuilder   { _p: [u8; 0] }
#[repr(C)] pub struct CpSolverResponse { _p: [u8; 0] }

#[repr(i32)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LpSolverType { Glop = 0 }

#[repr(C)] pub struct MpSolver { _p: [u8; 0] }
...
extern "C" {
    ...
    pub fn cpmodel_new_int_var(m: *mut CpModelBuilder, lb: i64, ub: i64,
                               name: *const c_char) -> i32;
    pub fn cpmodel_new_bool_var(m: *mut CpModelBuilder, name: *const c_char) -> i32;
    pub fn cpmodel_add_linear_le(m: *mut CpModelBuilder, idx: *const i32,
                                 c: *const i64, n: usize, rhs: i64);
    pub fn cpmodel_add_linear_ge(m: *mut CpModelBuilder, idx: *const i32,
                                 c: *const i64, n: usize, rhs: i64);
    pub fn cpmodel_add_linear_eq(m: *mut CpModelBuilder, idx: *const i32,
                                 c: *const i64, n: usize, rhs: i64);
    ...
    pub fn mpsolver_num_var(s: *mut MpSolver, lb: c_double, ub: c_double,
                            name: *const c_char) -> i32;
    pub fn mpsolver_int_var(s: *mut MpSolver, lb: c_double, ub: c_double,
                            name: *const c_char) -> i32;
    pub fn mpsolver_bool_var(s: *mut MpSolver, name: *const c_char) -> i32;
    pub fn mpsolver_add_constraint(s: *mut MpSolver, lb: c_double, ub: c_double,
                                   name: *const c_char) -> i32;
...
pub mod safe {
    ...
    impl CpModel {
        pub fn new() -> Self {
            unsafe {
                Self { ptr: NonNull::new(cpmodel_new()).expect("cpmodel_new returned null") }
            }
        }
        ...
        pub fn add_linear_le(&mut self, vars: &[i32], coeffs: &[i64], rhs: i64) {
            ...
            unsafe { cpmodel_add_linear_le(self.ptr.as_ptr(), vars.as_ptr(), coeffs.as_ptr(), vars.len(), rhs) }
        }

        pub fn add_linear_ge(&mut self, vars: &[i32], coeffs: &[i64], rhs: i64) {
            assert_eq!(vars.len(), coeffs.len());
            unsafe { cpmodel_add_linear_ge(self.ptr.as_ptr(), vars.as_ptr(), coeffs.as_ptr(), vars.len(), rhs) }
        }

        pub fn add_linear_eq(&mut self, vars: &[i32], coeffs: &[i64], rhs: i64) {
            assert_eq!(vars.len(), coeffs.len());
            unsafe { cpmodel_add_linear_eq(self.ptr.as_ptr(), vars.as_ptr(), coeffs.as_ptr(), vars.len(), rhs) }
        }

        pub fn add_all_different(&mut self, vars: &[i32]) {
            unsafe { cpmodel_add_all_different(self.ptr.as_ptr(), vars.as_ptr(), vars.len()) }
        }

        pub fn minimize(&mut self, vars: &[i32], coeffs: &[i64]) {
            assert_eq!(vars.len(), coeffs.len());
            unsafe { cpmodel_minimize(self.ptr.as_ptr(), vars.as_ptr(), coeffs.as_ptr(), vars.len()) }
        }

        pub fn maximize(&mut self, vars: &[i32], coeffs: &[i64]) {
            assert_eq!(vars.len(), coeffs.len());
            unsafe { cpmodel_maximize(self.ptr.as_ptr(), vars.as_ptr(), coeffs.as_ptr(), vars.len()) }
        }
    ...
    impl Drop for CpModel {
        fn drop(&mut self) { unsafe { cpmodel_free(self.ptr.as_ptr()) } }
    }
    ...
    impl Drop for CpSolution {
        fn drop(&mut self) { unsafe { cpresponse_free(self.ptr.as_ptr()) } }
    }
    ...
    impl LinearSolver {
        ...
        pub fn maximize(&mut self) { unsafe { mpsolver_maximize(self.ptr.as_ptr()) } }
        pub fn minimize(&mut self) { unsafe { mpsolver_minimize(self.ptr.as_ptr()) } }
    ...
    impl Drop for LinearSolver {
        fn drop(&mut self) { unsafe { mpsolver_free(self.ptr.as_ptr()) } }
    }
}
...
mod tests {
    ...
    mod integration {
        ...
        fn cpsat_all_different() {
            let mut m = CpModel::new();
            let a = m.new_int_var(1, 3, "a");
            let b = m.new_int_var(1, 3, "b");
            let c = m.new_int_var(1, 3, "c");
            m.add_all_different(&[a, b, c]);
            let s = m.solve(60.0);
            assert!(s.status().is_success());
            let vals = [s.value(a), s.value(b), s.value(c)];
